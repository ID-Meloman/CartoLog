...
def parse_car_page(url):
    try:
        ...
        if title_tag and title_tag.text:
            ...
            # Размер дисков
            wheel_size_row = soup.find('tr', text=lambda t: t and 'Размер дисков' in t)
            if wheel_size_row:
                wheel_size = wheel_size_row.find_next('td').text.strip()
                wheel_size = int(wheel_size[1:]) if wheel_size.startswith('R') and wheel_size[1:].isdigit() else 0
            else:
                wheel_size = '0'

            # Светодиодные фары
            led_headlights_row = soup.find('tr', text=lambda t: t and 'Фары' in t)
            ...
            fog_lights_row = soup.find('tr', text=lambda t: t and 'Противотуманные фары' in t)
            if fog_lights_row:
            ...
            # Тонированные стекла
            tinted_windows_row = soup.find('tr', text=lambda t: t and 'Тонированное остекление' in t)
            ...
            # Рейлинги на крыше
            roof_rails_row = soup.find('tr', text=lambda t: t and 'Система перевозки багажа на крыше' in t)
            if roof_rails_row:
